...
```hcl
module "s3" {
    source = "github.com/pbs/terraform-s3-module?ref=x.y.z"
}
```
...
gh -R 'pbs/terraform-s3-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/s3'
rm -f release.tar.gz
```
...
```bash
git remote add -f s3 git@github.com:pbs/terraform-s3-module.git
git subtree add --prefix terraform/modules/s3 s3 main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/s3 s3 $REF --squash
```
